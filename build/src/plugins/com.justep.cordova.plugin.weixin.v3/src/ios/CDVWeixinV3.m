...
-(void)pluginInitialize{
    ...
    self.app_secret = [viewController.settings objectForKey:@"weixin_secret"];
...
- (void)auth:(CDVInvokedUrlCommand *)command{
    ...
    [self sendAuthRequestScope: kAuthScope
                                        State:kAuthState
                                       OpenID:nil
...
-(instancetype)sharedManager {
    ...
    static CDVWeixinV3 *instance;
    dispatch_once(&onceToken, ^{
        instance = [[CDVWeixinV3 alloc] init];
    });
    return instance;
}
...
//获取参数信息
- (void)getParam:(CDVInvokedUrlCommand *)command{
    NSDictionary *paramDic = @{@"weixin_appid":self.app_id,@"weixin_secret":self.app_secret,@"weixin_partner_id":self.partner_id};
    CDVPluginResult *result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:paramDic];
    [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
...
- (void)onResp:(BaseResp *)resp{
    ...
    }else if ([resp isKindOfClass:[PayResp class]])
    {
        ...
        NSDictionary *resultDic = @{@"code":code,@"state":state};
